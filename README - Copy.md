...
Streamlit deployment layer for **SIFT** (Supply-chain
...
- **Assess** — pick or hand-build a package, run the pipeline, get a verdict
  card with the triage rail, attack-surface profile, and evidence.
- **Review queue** — the analyst queue produced by the current policy.
- **Policy** — the sidebar thresholds are product policy dials; metrics and the
  automation-vs-workload curve recompute live on the held-out test set.
- **Monitoring** — score distributions, archetype mix, feature importance.
